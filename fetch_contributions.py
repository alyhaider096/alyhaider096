...
def parse_days(html: str):
    ...
    # Modern GitHub markup uses <td> cells with data-date / data-level,
    # or <rect> hover cards with a `data-date`/`data-level`. Newer pages
    # ship a <tool-tip> + <td>/<rect> pair; handle both shapes defensively.
    cells = soup.select("td.ContributionCalendar-day, rect.ContributionCalendar-day")

    if not cells:
        # Fallback: some responses use <td> with an id and an aria-label like
        # "No contributions on January 1st." — walk every td with data-date.
...
def derive_stats(days):
    ...
    total = None
    # Grab the literal "N contributions in the last year" heading if present.
    return {
        "total_days_recorded": len(days),
    }
...
def main():
    ...
    # Total contributions in the last year, straight from the page heading.
    soup = BeautifulSoup(html, "html.parser")
